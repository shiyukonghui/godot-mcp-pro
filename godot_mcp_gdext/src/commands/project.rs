...
fn cmd_project_path_to_uid(args: &serde_json::Map<String, serde_json::Value>) -> Result<serde_json::Value, McpError> {
    ...
    // 使用 ResourceUID.get_id_for_path 获取 ID，再 id_to_text 转成文本
    let code = format!("var uid = ResourceUID::get_singleton(); return uid.id_to_text(uid.get_id_for_path('{}'))", path.replace('\'', "\\'"));
    let mut expr = godot::classes::Expression::new_gd();
    if expr.parse(&code) == godot::global::Error::OK {
        let result = expr.execute();
        if let Ok(ref text) = result.try_to::<String>() {
            if !text.is_empty() {
                return Ok(serde_json::json!({"path": path, "uid": text}));
            }
        }
    }
